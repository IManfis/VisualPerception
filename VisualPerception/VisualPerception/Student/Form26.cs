...
namespace VisualPerception.Student
{
    public partial class Form26 : Form
    {
        ...
        private void Write(int word, int _id)
        {
            ...
            var reproducedIncentive = context.Experiment3Result.First(x => x.Id == _id).ProvidedIncentive.Split(new char[] { ',' },
                StringSplitOptions.RemoveEmptyEntries).ToList();

            var reproducedIncentiveInt = context.Experiment3Result.First(x => x.Id == _id).Hallmark.Split(new char[] { ',' },
                StringSplitOptions.RemoveEmptyEntries).ToList();
